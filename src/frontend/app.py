...
st.markdown(f"""
<div class="bike-card">
    <strong style="color:#1D9E75;">{bike_choice}</strong> &nbsp;·&nbsp; {bike['wh']}<br>
    {bike['desc']}<br>
...
with col2:
    battery = st.slider("Battery level (%)", 10, 100, 80)

distance_km = distance * 1.60934 if unit == "miles" else distance

# ── Warnings ──────────────────────────────────────────────────────────
if battery < 30:
    st.warning("⚠️ Low battery — your trip may take considerably longer.")

if distance > long_threshold:
    st.warning(
        f"⚠️ You've selected a long distance ({distance} {unit}). "
...
if st.button("Predict my trip duration", type="primary", use_container_width=True):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if mins < 30:
                st.success("🏎️ **Fast & Furious** — You'll be there in no time!")
                st.balloons()
